...
def _enrich_tracks_with_isrcs(
    ...
    # 1. Expand the persistent session's connection pool to match worker scaling.
    session = requests.Session()
    adapter = requests.adapters.HTTPAdapter(pool_connections=20, pool_maxsize=20)
    session.mount('https://', adapter)
    session.mount('http://', adapter)

    # 2. Define an isolated worker target targeting an atomic task return
    def fetch_single(track):
        ...
        tid = uri.split(":")[-1]
        try:
            # Reuses the thread-safe pool directly; token passed as a static primitive
            isrc = get_track_isrc(tid, headers=token, session=session)
            return track, isrc
        except Exception:
            return track, ""

    # 3. Process the collection concurrently without internal dictionary mutations
    with ThreadPoolExecutor(max_workers=20, thread_name_prefix="ISRCWorker") as executor:
        # executor.map naturally maintains order and avoids manual chunk manipulation
        results = executor.map(fetch_single, tracks)

    # 4. Construct and apply the final data mapping safely outside the loop windows
...
class SpotifyGraphQLSDK:
    ...
    def track(
        ...
        # Resolve ISRC for the single track
    ...
    def album(
        ...
        # Resolve ISRCs for all album tracks
    ...
    def artist(
        ...
        # Enrich top tracks with duration (artist query omits it)
