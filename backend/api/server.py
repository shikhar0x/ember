    ...
    def warmup():
        try:
            ...
            try:
                from core.spotify import _tm
                _tm._warmup_done = False
                _tm._needs_login = True
                _tm._warmup_message = f"Failed to harvest token: {e}"
            except Exception:
                pass
...
async def lifespan(app: FastAPI):
    ...
    # Start FFmpeg acquisition in background
    ffmpeg_mgr = get_ffmpeg_manager()
    ffmpeg_mgr.start_acquisition()
    yield                 
...
def create_app(
    ...
    async def health():
        ...
        ffmpeg_mgr = get_ffmpeg_manager()
        if not ffmpeg_mgr.is_ready:
            raise HTTPException(status_code=503, detail="FFmpeg not ready")
    ...
    async def status():
        from core.spotify import _tm
        ffmpeg_mgr = get_ffmpeg_manager()
        ffmpeg_status = ffmpeg_mgr.get_status()
        return {
    ...
    @_app.post("/retry_login")
    async def retry_login():
        from core.spotify import _tm
        ffmpeg_mgr = get_ffmpeg_manager()
        if ffmpeg_mgr.is_failed:
            ffmpeg_mgr.start_acquisition()
        with _tm._lock:
            _tm._bearer = None
            _tm._expires_at = 0
            ...
            _tm._client_token_expires_at = 0.0
            _tm._warmup_done = False
            _tm._needs_login = False
            _tm._warmup_message = "Connecting to Spotify..."
...
def start_background(
    ...
    _server_thread = threading.Thread(target=server.run, daemon=True, name="ember-api")
