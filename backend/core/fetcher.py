...
def fetch_audio(video_id: str, output_dir: str = "downloads", progress_hook=None,
    ...
    for fmt in formats_to_try:
        try:
            ...
            print(f"[Ember] Download succeeded using format: {fmt} -> {os.path.basename(raw_path)}")
            break

        except Exception as e:
            print(f"[Ember] Format '{fmt}' failed: {e}")
            last_error = e
            continue

    if not info:
        raise RuntimeError(f"[Ember] All extraction strategies failed: {last_error}")
