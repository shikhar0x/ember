    ...
    try:
        ...
        if final_path.exists():
            try:
                ...
                pass
        for p in dest.glob(f"{safe_title}*"):
            try:
                if p.is_file():
                    os.remove(str(p))
            except OSError:
...
def download_track_manual(
    ...
    """Same pipeline as download_track, but skips resolve() — the audio
    source is a user-supplied YouTube/YTMusic URL instead of auto-matched."""
    audio_codec = options.get("audio_codec", "mp3")
    audio_quality = options.get("audio_quality", "0")
    ext = _CODEC_EXT.get(audio_codec, audio_codec)

    artist_str = ", ".join(dict.fromkeys(spotify_track.artists)) if spotify_track.artists else "Unknown"
    safe_title = core_sanitize(f"{spotify_track.title} - {artist_str}")
    final_path = _unique_path(dest, safe_title, ext)

    video_id = _extract_youtube_video_id(youtube_url)
    if not video_id:
        emit(callback, error_event("Couldn't parse a video ID from that URL"))
        return False
    ...
    try:
        ...
        def _hook(d):
            ...
            if d["status"] == "downloading" and callback:
                ...
                if total > 0:
                    ...
                    p_num = "".join(c for c in p_str_clean if c.isdigit() or c == ".")
                    try:
                        pct = float(p_num) / 100.0 if p_num else 0.0
                    except ValueError:
                        pct = 0.0
        ...
        emit(callback, progress_event(1.0, "Embedding metadata..."))
        cover_bytes = _resolve_cover(spotify_track, yt_thumb)
        ...
        print(f"[SpotifyDL] download_track_manual error: {e}")
        emit(callback, error_event(f"Failed: {e}"))
        return False
